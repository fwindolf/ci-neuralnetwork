...
double mean_in(struct data **d, int d_len, int idx)
{
    double sum = 0.0;
    for (int i = 0; i < d_len; i++)
    {        
...
double * normalize_input(struct data **d, int d_len, struct data **c, int c_len)
{
    ...
    // Concat the data
    int all_len = d_len + c_len;
    struct data ** all = calloc(all_len, sizeof(struct data *));
    for(int i = 0; i < d_len; i++)
        all[i] = d[i];
    for(int i = d_len; i < d_len + c_len; i++)
        all[i] = c[i - d_len];
    ...
    for (int i = 0; i < INPUT_UNITS  - 1; i++)
    {
        ...
        debug("Normalizing Input Channel %d with mean %.6lf\n", i, mean);
    ...
    for (int i = 0; i < INPUT_UNITS  - 1; i++)
    {
        double min = min_in(all, all_len, i);
        double max = max_in(all, all_len, i);
...
{
    double * norms = calloc(OUTPUT_UNITS, sizeof(double));
    ...
    for (int i = 0; i < OUTPUT_UNITS; i++)
    {
        ...
        max = fabs(min) > fabs(max) ? fabs(min) : fabs(max);
        debug("Normalizing Output Channel %d with factor %.6lf\n", i, max);
        for (int j = 0; j < d_len; j++)
        {
            d[j]->out[i] /= max;            
        }
...
void backwards_propagation(struct network *n, struct data *d)
{
    ...
    {      
        n->output_layer->delta[i] = d->out[i] - n->output_layer->outputs[i]; // Target - Actual
...
int main(void)
{
    ...
    double * out_norms = normalize_output(all, d_len);
    ...
    for (int i = 0; i < c_len; i++)
    {
        ...
        info("%.6lf - ", out[0]);
        output("%.6lf\n", out_norms[0] * out[0]); // Scale with normalization factor
    }
}
